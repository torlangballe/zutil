...
func (n *NetStats) listenMDNS(ctx context.Context) {
...
func (n *NetStats) sendMdns(ip IP, mhaddr net.HardwareAddr) {
	...
	if err != nil {
		zlog.Fatal("Serialize layers出现问题:", err)
	}
	outgoingPacket := buffer.Bytes()

	handle, err := pcap.OpenLive(n.iface, 1024, false, 10*time.Second)
	if err != nil {
		zlog.Fatal("pcap打开失败:", err)
	}
	defer handle.Close()
	err = handle.WritePacketData(outgoingPacket)
	if err != nil {
		zlog.Fatal("writePacket...")
	}
}
